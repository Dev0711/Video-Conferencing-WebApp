const joinRoom = () => {
    socketRef.current.emit("joinRoom", { meetingId }, (data) => {
        console.log(`Router RTP Capabilities... ${data.rtpCapabilities}`);
        // we assign to local variable and will be used when
        // loading the client Device (see createDevice above)
        // rtpCapabilities = data.rtpCapabilities;
        setRtpCapabilities((prev) => (prev = data.rtpCapabilities));

        // once we have rtpCapabilities from the Router, create Device

        if (socketRef.current) createDevice();
    });
};
...
const connectSendTransport = async () => {
    ...
    // audioProducer = await producerTransport.produce(audioParamsRef.current);
    setAudioProducer(producerTransport.produce(audioParamsRef.current));
    // videoProducer = await producerTransport.produce(videoParamsRef.current);
